...
typedef enum portMode_t {
    ...
    MODE_INVERTED     = 1 << 5,
    MODE_STOPBITS2    = 1 << 6,
    MODE_PARITY_EVEN  = 1 << 7,
// driver specific modes below
// softserial specific
    MODE_S_DUALTIMER  = 1 << 10, // try to claim adjacent timer channel in softserial mode
// uart specific
    MODE_U_DMARX      = 1 << 12, // use USART RX DMA if available
    MODE_U_DMATX      = 1 << 13, // use USART TX DMA if available
    MODE_U_REMAP      = 1 << 14,// remap USART to alternate pins
