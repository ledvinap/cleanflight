...
# if 0
    static const sonarHardware_t sonarPWM56 = {
        .triggerIO = DEFIO_REC(PB8),   // PWM5 (PB8) - 5v tolerant
        .echoIO = DEFIO_REC(PB9)       // PWM6 (PB9) - 5v tolerant
    };
    static const sonarHardware_t sonarRC78 = {
        .triggerIO = DEFIO_REC(PB0),   // RX7 (PB0) - only 3.3v ( add a 1K Ohms resistor )
        .echoIO = DEFIO_REC(PB1),      // RX8 (PB1) - only 3.3v ( add a 1K Ohms resistor )
    };
...
# else
    static const sonarHardware_t const sonarHardware = {
        .triggerIO = DEFIO_REC(PB7),
        .echoIO = DEFIO_REC(PB6),
    };
...
# endif
    ...
    UNUSED(batteryConfig);
    static const sonarHardware_t const sonarHardware = {
        .triggerIO = DEFIO_REC(PB0),   // RC_CH7 (PB0) - only 3.3v ( add a 1K Ohms resistor )
        .echoIO = DEFIO_REC(PB1),       // RC_CH8 (PB1) - only 3.3v ( add a 1K Ohms resistor )
    };
    hcsr04_init(&sonarHardware);
#else
