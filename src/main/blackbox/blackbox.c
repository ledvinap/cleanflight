...
#ifdef NEWACCGYRO
    for(int i = 0; i < 4; i++)
        blackboxWriteSigned16VBArray(blackboxCurrent->gyroDataHistory[i], XYZ_AXIS_COUNT);
#endif
...
#ifdef NEWACCGYRO
    for(int i = 0; i < 4; i++)
        blackboxWriteMainStateArrayUsingAveragePredictor(offsetof(blackboxMainState_t, gyroDataHistory[i]),   XYZ_AXIS_COUNT);
#endif
